"""
nlp_module.py — NLP Sentiment & Linguistic Analysis
=====================================================
...
────────────────────────────────────────────────────────
    ...
    # result is an NLPOutput dataclass — see nlp_schemas.py
...
class NLPAnalyzer:
    ...
    def analyze(self, text: str) -> NLPOutput:
        ...
        text : str
            ...
            Example:
                "I've been feeling really anxious lately.
                 My sleep has been terrible. I keep waking up."

        Returns
        -------
        NLPOutput
            See nlp_schemas.py for full field documentation.
        ...
        -----------------------
            ...
            if result.error:
                # log it, return a degraded response
                return handle_nlp_failure(result.error)

            if result.low_data_warning:
                # scores exist but confidence is low
                confidence = 0.3
            else:
                confidence = 0.9

            # Use result.anxiety_score in fusion algorithm
            fusion_score = compute_fusion(
                acoustic_score,
                result.anxiety_score,
                result.pronoun_density
            )
        """
        # ── Guard: empty input ────────────────────────────────
    ...
    def _classify_emotions(self, text: str) -> EmotionScores:
        """
        Run the transformer emotion classifier.
        ...
        Averaging probabilities is mathematically correct here because
        each chunk is an independent sample from the same speaker.
        It's equivalent to asking "on average, what emotion does this
        person express?" across the whole transcript.
        """
        ...
        for chunk in chunks:
            ...
            # result[0] is the list of {label, score} dicts for this input
        ...
        # Map to our EmotionScores dataclass
        # Note: model uses lowercase labels matching our field names
    ...
    def _chunk_text(self, text: str) -> List[str]:
        ...
        Example with CHUNK_SIZE=5, OVERLAP=2 on "a b c d e f g h":
            chunk 1: "a b c d e"
            chunk 2: "d e f g h"   ← reuses last 2 words of chunk 1
        """
        words = text.split()
    ...
    def _compute_pronoun_density(self, text: str) -> float:
        ...
        Implementation detail:
            We lowercase and strip punctuation before counting to
            catch "I," and "me." as well as "I" and "me".
        """
    ...
    def _compute_sentence_complexity(self, text: str) -> float:
        ...
        Why this matters clinically:
            Fragmented speech (very short sentences, avg < 5 words)
            can indicate cognitive disruption associated with anxiety.
            Overly long run-on sentences (avg > 25 words) can indicate
            pressured, disorganized speech.

        Sentence splitting:
            We split on ., !, ? followed by whitespace or end-of-string.
            This handles most conversational text without needing NLTK.
        """
